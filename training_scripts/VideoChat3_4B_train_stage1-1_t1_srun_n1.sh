...
# 需要先srun -p videop1 -N2 -n2 --job-name=vdebug --quotatype=spot --ntasks-per-node=1 --cpus-per-task=128 --gres=gpu:8
set -ex
nnodes=1
...
TASK_NAME="VideoChat3_4B_train_stage1-1_t1"
...
#run commandVideoChat3Dense4BT1Config
torchrun --nnodes=$nnodes \
        --nproc_per_node=8 \
        --rdzv_backend=c10d \
        --rdzv_endpoint=${rdzv_endpoint} \
        xtuner/v1/train/cli/sft.py --config training_configs/videochat3/VideoChat3_4B_train_stage1-1_t1.py 2>&1 | tee -a "${OUTPUT_DIR}/training_log_${TASK_NAME}_${current_time}.txt"
